...
fileName: String,
  ...
  def getBatchSizeAndNumBatches(N: Int, numUniqueKeys: Int): (Int, Int) = {
    val batchSize = if (batchSizeConf == "numUniqueKeys") {
      println(s"[Config] Auto-set the batchSize to be the numUniqueKeys = $numUniqueKeys")
      numUniqueKeys
    } else {
    ...
    println(Console.CYAN + s"[Config] Total number of batches = N/batchSize = $numBatches" + Console.RESET) 
...
object RyaConfig {
  ...
  def load(): RyaConfig = {
    ...
    val fileName = sys.props.getOrElse("INPUT_FILE", "Example_Rya_Input.txt")
