...
parser.add_argument('--filename', type=str, default="identical_int.txt", help='Output filename')
...
# ================= Generate Data =================
def generate_data():
    print(f"Generating EXACTLY {TARGET_SIZE_MB}MB ({TARGET_SIZE_BYTES} bytes) of data ofRepeated '{FIXED_INT.strip()}'")
    ...
    with open(FILENAME, "w", encoding="utf-8") as f:
        ...
        # ================= Step 1: Write all other integers once =================
        print(f"--> Writing all integers once (from 0 to {NUM_UNIQUE_KEYS-1})")
        for i in range(0, NUM_UNIQUE_KEYS):
            token = str(i) + " "
            token_len = len(token)
            f.write(token)
            current_bytes += token_len
        
        # ================= Step 2: Write fixed integer =================
