...
object DataLoader {
  ...
  // --- Load Int Array ---
  def loadIntData(inputDataStrings: Array[String]): (Array[Int], Array[Int], Int) = {
    println(Console.YELLOW + "[DataLoader] Reading data as raw Ints..." + Console.RESET)
  ...
  def saveOutput(histogram: Map[Int, Int], baseName: String): Unit = {
    ...
    // Save
    val outputFile = new File(s"${baseName}_output_int.txt")
