  ...
  def runOLABEL(sortedKeys: Array[Int], B: Int): Array[BinItem] = {
    ...
    for (i <- 1 until n) {
      ...
      val label = if (currentKey == prevKey) {
        ...
        // --- Print OLABEL Assignment ---
        val idxBin = HelpPrinter.toBinaryStr(i, levels)
        val lblBin = HelpPrinter.toBinaryStr(newLabel, levels)
        println(Console.GREEN + f"    [OLABEL] Key '$currentKey' -> Index $i%2d ($idxBin) -> RevLex -> Label $newLabel%2d ($lblBin)" + Console.RESET)
